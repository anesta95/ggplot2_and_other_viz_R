...
DF2 <- as.data.frame.matrix(table(adult$SRAGE_P, adult$RBMI))

# Create groupSum, xmax and xmin columns
DF2$groupSum <- rowSums(DF2)
DF2$xmax <- cumsum(DF2$groupSum)
DF2$xmin <- DF2$xmax - DF2$groupSum
...
# Melt the dataset
DF_melted2 <- melt(DF2, id.vars = c("X", "xmin", "xmax"), variable.name = "FILL")
...
ggplot(DF_melted2, aes(ymin = ymin,
                      ymax = ymax,
                      xmin = xmin,
                      xmax = xmax,
                      fill = FILL)) +
